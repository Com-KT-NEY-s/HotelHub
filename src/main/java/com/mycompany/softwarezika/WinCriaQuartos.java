...
public class WinCriaQuartos extends javax.swing.JFrame {
    ...
    public WinCriaQuartos() {
        ...
        if (conn == null) {
            JOptionPane.showMessageDialog(rootPane, "Goze o Champions!");
        }
    }
    private void carregaTabela() {
        try {
            ...
            String[][] quartos = quarto.ler();


            DefaultTableModel tableModelQ = new DefaultTableModel(quartos, colunasQ);
            tableModelQ.setColumnIdentifiers(colunasQ);

            tableModelQ.setRowCount(0);
            for (String[] linha : quartos) {
                tableModelQ.addRow(linha);
            }

            tblCriaQuartos.setModel(tableModelQ);
        } catch (Exception e) {
    ...
    private void btnAdicionarQuartosActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAdicionarQuartosActionPerformed
        Classes.Quartos quarto = new Classes.Quartos();
